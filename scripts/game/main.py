...
# For debugging class not found from pfscene: #
# print(sys.modules)
# import sys, inspect
# def print_classes(matchesFilter, module=inspect.getmembers(sys.modules[__name__])):
#     for name, obj in module:
#         if inspect.isclass(obj) and matchesFilter(name):
#             print(obj)
# m = inspect.getmembers(sys.modules['units.sinbad'])
# print(m)
# print_classes(lambda x: True, module=m)
# print_classes(lambda c: c == "Blagostrazh", module=m)
# print_classes(lambda c: c == "Sinbad", module=m)
#raw_input()
# #
def load_scene():
    globals.scene_objs, globals.scene_regions = pf.load_scene("assets/maps/dop.pfscene")
load_scene()

# Set which faction indices are enemies ("war") (allies is another constant I think)
...
# Factions controllable or not currently:
pf.set_faction_controllable(0, False) # Mother Nature
pf.set_faction_controllable(1, False) # Sinbad's Forces  ( -> True if you play as them)
pf.set_faction_controllable(2, False) # Goblin Confederacy (like enemy civilians in supcom)
pf.set_faction_controllable(3, False) # Arcadians  ( -> True if you play as them)
