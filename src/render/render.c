...
//#endif
    ...
    while(true) {
        ...
        if(rstate->swap_buffers) {
            ...
            #ifdef __APPLE__
            // Run `update` part of `SDL_GL_SwapWindow(window);` (which does [SDLOpenGLContext update] and [SDLOpenGLContext flushBuffer], see SDL/src/video/cocoa/SDL_cocoaopengl.m under `Cocoa_GL_SwapWindow`) on main thread instead:
            SDLOpenGLContext* nscontext = (SDLOpenGLContext*)SDL_GL_GetCurrentContext();
            CFStringRef aCFString;
            aCFString = CFStringCreateWithCString(NULL, "flushBuffer", kCFStringEncodingUTF8);
            ((void(*)(void*, SEL))objc_msgSend)(nscontext, NSSelectorFromString(aCFString)); // Call the `flushBuffer` method on the current SDLOpenGLContext object (which inherits from NSOpenGLContext : https://developer.apple.com/documentation/appkit/nsopenglcontext/1436211-flushbuffer?language=objc )
            CFRelease(aCFString);
            #else
